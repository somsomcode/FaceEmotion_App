...
// 1. 서버로 고정된 값 설정
const finalResult = {
    emotion: 4, // 고정된 감정 값 3
    weightedAverageScore: 78.00 // 고정된 가중 평균 점수 78%
};

// 2. 최종 감정 분석 결과 출력
console.log(`최종 감정: ${finalResult.emotion}, 가중 평균 점수: ${finalResult.weightedAverageScore}`);

// 3. 로컬 스토리지에 최종 감정 분석 결과 저장 (서버로 전송할 값)
localStorage.setItem('finalEmotionResult', JSON.stringify(finalResult));
console.log(finalResult);

// 4. 서버로 데이터를 전송하는 함수 (필요 시 추가)
async function sendToServer() {
    try {
        const response = await fetch("https://your-server-url.com/endpoint", {
            ...
            body: JSON.stringify(finalResult), // 고정된 값 전송
        });
